...
<title>Verifikasi Kode — {{ config('app.name', 'Lumora Hosting') }}</title>
...
<body class="antialiased font-sans bg-slate-50 min-h-screen flex items-center justify-center p-6">
  ...
  <div class="w-full max-w-sm">
    <div class="flex items-center gap-3 mb-8 justify-center">
      @php $vcLogo = \App\Models\Setting::get('site_logo'); @endphp
      @if ($vcLogo)
        <img src="{{ route('branding.file', $vcLogo) }}" alt="{{ config('app.name', 'Lumora Hosting') }}" class="h-11 w-auto object-contain">
      @else
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
      ...
      <form method="POST" action="{{ route('admin.password.verify.code') }}" class="space-y-4">
        @csrf
        <div>
    ...
    <p class="text-center text-sm text-slate-500 mt-6">
      Tidak menerima kode?
      <a href="{{ route('admin.password.request') }}" class="text-accent font-medium hover:underline">Kirim ulang</a>
    </p>
  </div>
