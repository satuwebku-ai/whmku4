...
class RegisterController extends Controller
{
    ...
    public function store(Request $request, CaptchaService $captcha, AffiliateReferralService $affiliateReferrals): RedirectResponse
    {
        ...
        // Kalau client ini datang lewat link referral affiliate (cookie
        // affiliate_ref), catat atribusinya sekarang -- gagal di sini
        // TIDAK BOLEH menggagalkan pendaftaran itu sendiri.
        try {
            $affiliateReferrals->attachToNewClient($client, $request);
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::warning('Atribusi affiliate gagal: ' . $e->getMessage());
        }
